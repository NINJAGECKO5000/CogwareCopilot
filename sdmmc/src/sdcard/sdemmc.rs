...
use crate::sdcard::sdemmc::rpi4_constants::*;
// use crate::time::{get_sys_tick_count, now, sleep};
use bcm2837_hal::delay::Timer;
...
pub struct EMMCController {
    // timer: RefCell<bcm2837_hal::delay::Timer>,
    registers: Registers,
}
...
impl BlockDevice for EMMCController {
    ...
    fn read(
        ...
        let res = self.emmc_transfer_blocks(start_block_idx.0, num_blocks as u32, &mut buff, false);
        match res {
            SdResult::EMMC_OK => Ok(()),
            _ => Err(res),
        }
...
impl EMMCController {
    ...
    // pub const fn new(timer: bcm2837_hal::delay::Timer) -> Self {
    ...
    pub fn emmc_wait_for_interrupt(&self, mask: u32) -> SdResult {
        ...
        // Data timeout occurred
        {
            ...
            info!(
                "EMMC: Error waiting for interrupt :{:b}, :{:b}, :{:b}\n",
                self.registers.EMMC_STATUS.get(),
                ival,
                self.registers.EMMC_RESP0.get()
            );
    ...
    pub fn emmc_send_app_command(&self) -> SdResult {
        ...
        if unsafe { EMMC_CARD.rca == 0 } {
            ...
            timer.delay_us(100); // add a 100 us delay for cmds that automatically send APP_CMDs
    ...
    pub fn emmc_reset_card(&self) -> SdResult {
        ...
        timer.delay_us(1); // Wait 1 microsecond
        self.registers
            .EMMC_CONTROL0
    ...
    pub fn emmc_init_card(&mut self) -> SdResult {
